...
object utils {
  ...
  def loadUnaryContextProvider(className: String, context: DMNContextPath, child: Expression): DMNContextProvider[_] = (
    for {
      ...
      constructor <- Try(clazz.getConstructor(classOf[DMNContextPath], classOf[Expression]))
      constructed <- Try(constructor.newInstance(context, child).asInstanceOf[DMNContextProvider[_]])
      provider = constructed
    } yield
      provider
  ).fold(t => throw DMNException(s"Could not loadUnaryContextProvider $className", t), identity)
